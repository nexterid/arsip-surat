...
(/** @lends <global> */function( $ ) {
	 ...
	 *    Hungarian value in the `user_old` object will be overwritten. Otherwise they
	 *    won't be.
	 *  @memberof DataTable#oApi
	 */
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		else
		{
			...
			/* Don't require that the user_old must specify bRegex, bSmart or bCaseInsensitive */
	...
	function _fnBuildHead( oSettings )
	{
		...
		// row in the footer. If there is more than one row the user_old wants to
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		/* Loop over the user_old set positioning and place the elements as needed */
	...
	/**
	 * Filter the data table based on user_old input and draw the table
	 *  @param {object} settings dataTables settings object
	...
	/**
	 * Generate the node required for user_old display length changing
	 *  @param {object} settings dataTables settings object
	...
	function _fnScrollDraw ( settings )
	{
		...
		if ( table.outerWidth() < sanityWidth )
		{
			...
			// And give the user_old a warning that we've stopped the table getting too small
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		/* Convert any user_old input sizes into pixel sizes */
		...
		else
		{
			...
			// data, assign any user_old defined widths, then insert it into the DOM and
			...
			// the table width should be by summing the user_old given values, and the
			// automatic values
			if ( scrollX )
			{
				var total = 0;
	...
	/**
	 * Function to run on user_old sort request
	 *  @param {object} settings dataTables settings object
	...
	function _fnSortListener ( settings, colIdx, append, callback )
	{
		...
		// callback used for async user_old interaction
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @param {int} level log error messages, or display them to the user_old
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			_fnCallbackReg( oSettings, 'aoDrawCallback',       oInit.fnDrawCallback,      'user_old' );
			_fnCallbackReg( oSettings, 'aoServerParams',       oInit.fnServerParams,      'user_old' );
			_fnCallbackReg( oSettings, 'aoStateSaveParams',    oInit.fnStateSaveParams,   'user_old' );
			_fnCallbackReg( oSettings, 'aoStateLoadParams',    oInit.fnStateLoadParams,   'user_old' );
			_fnCallbackReg( oSettings, 'aoStateLoaded',        oInit.fnStateLoaded,       'user_old' );
			_fnCallbackReg( oSettings, 'aoRowCallback',        oInit.fnRowCallback,       'user_old' );
			_fnCallbackReg( oSettings, 'aoRowCreatedCallback', oInit.fnCreatedRow,        'user_old' );
			_fnCallbackReg( oSettings, 'aoHeaderCallback',     oInit.fnHeaderCallback,    'user_old' );
			_fnCallbackReg( oSettings, 'aoFooterCallback',     oInit.fnFooterCallback,    'user_old' );
			_fnCallbackReg( oSettings, 'aoInitComplete',       oInit.fnInitComplete,      'user_old' );
			_fnCallbackReg( oSettings, 'aoPreDrawCallback',    oInit.fnPreDrawCallback,   'user_old' );
	...
	DataTable.defaults = {
		 ...
		 * cannot be overridden by user_old interaction with the table. What this means
		 * is that you could have a column (visible or hidden) which the sorting
		 * will always be forced on first - any sorting after that (from the user_old)
		 ...
		 * that it allows the end user_old to input multiple words (space separated) and
		...
		/**
		 * Allows the end user_old to select the size of a formatted page from a select
		 ...
		 * same time (activated by shift-click by the user_old).
		 *  @type boolean
		 *  @default true
		 *
		 *  @dtopt Options
		 ...
		 * display length, filtering and sorting. As such when the end user_old reloads
		 ...
		 * rendered as "1,000,000") to help readability for the end user_old. This
		 ...
		 * feature enabled (`lengthChange`) then the end user_old will be able to override
		...
		/**
		 * All strings that DataTables uses in the user_old interface that it creates
		...
		"oLanguage": {
			...
			"oPaginate": {
				 ...
				 * button to take the user_old to the first page.
				 *  @type string
				 *  @default First
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use when using the 'full_numbers' type of pagination for the
				 * button to take the user_old to the last page.
				 *  @type string
				 *  @default Last
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use for the 'next' pagination button (to take the user_old to the
				 * next page).
				 *  @type string
				 *  @default Next
				 *
				...
				/**
				 * Text to use for the 'previous' pagination button (to take the user_old to
				 * the previous page).
				 *  @type string
				 *  @default Previous
				 *
			...
			/**
			 * This string gives information to the end user_old about the information
			...
			/**
			 * When a user_old filters the information in a table, this string is appended
			 ...
			 * indicate to the end user_old the the data is being loaded. Note that this
			...
			/**
			 * Text which is displayed when the table is processing a user_old action
			...
			/**
			 * Details the actions that will be taken when the user_old types into the
	...
	DataTable.defaults.column = {
		 ...
		 *    // (which has both forms) is used for filtering for if a user_old inputs either format, while
		 ...
		 * version) prior to ordering. This allows ordering to occur on user_old
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			...
			/**
			 * Present a user_old control allowing the end user_old to change the page size
			 ...
			 * user_old request - typically an Ajax request for server-side processing.
			 * Note that this parameter will be set by the initialisation routine. To
			 * set a default use {@link DataTable.defaults}.
			 *  @type boolean
			 */
		...
		/**
		 * List of options that can be used for the user_old selectable length menu.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @type array
		 *  @default []
	...
	DataTable.ext = _ext = {
		 ...
		 *    // from the end-user_old, matching the data in a certain range.
	 ...
	 * the end user_old that there is something happening, or that something has
	 * finished.
